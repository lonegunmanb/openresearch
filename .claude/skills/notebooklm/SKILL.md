...
**IMPORTANT:** Before using any command, you MUST be authenticated.

### Login Using site-login Skill (Required)

⚠️ **DO NOT use `notebooklm login` command.** Instead, follow the `site-login` skill workflow to login `https://notebooklm.google.com/`. The `site-login` skill shares the same `./browser_profile` directory, so the authenticated session will be available to the `notebooklm` CLI commands.

After login, verify authentication works:
```bash
notebooklm list           # Should list notebooks without error
```

If commands fail with authentication errors, use `site-login` skill to re-authenticate.

### Browser Profile Directory (Required for This Workspace)

**CRITICAL:** When executing ANY `notebooklm` command in this workspace, you MUST set the `NOTEBOOKLM_HOME` environment variable to use `./browser_profile` as the browser profile directory. This ensures authentication state is stored locally in the workspace.

**PowerShell (Windows):**
```powershell
$env:NOTEBOOKLM_HOME = "./browser_profile"; notebooklm <command>
```

**Bash/Zsh (Linux/macOS):**
```bash
NOTEBOOKLM_HOME=./browser_profile notebooklm <command>
```

**Examples:**
```powershell
# Login
$env:NOTEBOOKLM_HOME = "./browser_profile"; notebooklm login

# List notebooks
$env:NOTEBOOKLM_HOME = "./browser_profile"; notebooklm list

# Create notebook
$env:NOTEBOOKLM_HOME = "./browser_profile"; notebooklm create "My Notebook"
```

⚠️ **DO NOT run any `notebooklm` command without setting `NOTEBOOKLM_HOME`**, otherwise authentication state will be stored in the default location (`~/.notebooklm`) instead of the workspace's `./browser_profile` directory.
